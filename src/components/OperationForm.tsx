...
import { Package, Calculator, ArrowRightLeft, DollarSign, Play, ListPlus } from 'lucide-react';
...
export const OperationForm: React.FC<OperationFormProps> = ({
  ...
  // Sincroniza o modo caso venha múltiplos itens do XML
  useEffect(() => {
    if (operacao.itens && operacao.itens.length > 1) {
      setModoMultiItem(true);
    }
  }, [operacao.itens]);
  ...
  const handleMultiItensChange = (newItens: ItemNotaFiscal[]) => {
    const totalOperacao = newItens.reduce((acc, curr) => acc + (curr.quantidade * curr.valor_unitario), 0);
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-slate-100">
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                NCMs, produtos, rota fiscal, valores da nota e finalidade de uso
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center bg-slate-100 p-1 rounded-lg border border-slate-200 text-xs font-semibold">
            <button
                ...
                className={`px-3 py-1 rounded transition cursor-pointer ${!modoMultiItem ? 'bg-white text-slate-900 shadow-2xs font-bold' : 'text-slate-500 hover:text-slate-800'}`}
            >
              Item Único
            </button>
            <button
                ...
                className={`px-3 py-1 rounded flex items-center gap-1.5 transition cursor-pointer ${modoMultiItem ? 'bg-emerald-800 text-white shadow-2xs font-bold' : 'text-slate-600 hover:text-slate-900'}`}
            >
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select
                  value={operacao.tipo_adquirente || 'ORGAO_PUBLICO_ESTADUAL'}
                  onChange={(e) => {
                  ...
                  className="w-full px-3.5 py-2.5 bg-white border border-slate-300 rounded text-xs font-semibold text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-600/20 focus:border-emerald-600"
              >
          ...
          <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-[10px] font-mono font-bold text-slate-600 uppercase tracking-widest mb-1 flex items-center gap-1">
                <ArrowRightLeft className="w-3 h-3 text-emerald-700" /> UF Origem (Saída Nota)
              </label>
              <select
                  value={operacao.uf_origem}
                  onChange={(e) => onChange({ uf_origem: e.target.value })}
                  className="w-full px-3 py-1.5 bg-white border border-slate-300 rounded text-xs font-semibold text-slate-900 focus:outline-none focus:ring-1 focus:ring-emerald-600"
              >
                {UFS.map(uf => (
            ...
            <div>
              ...
              <input
                  ...
                  disabled
                  className="w-full px-3 py-1.5 bg-emerald-50 border border-emerald-200 rounded text-xs font-bold text-emerald-900 text-center"
              />
            </div>
          </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select
                      ...
                      className="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded text-xs text-slate-800 font-medium focus:outline-none focus:ring-2 focus:ring-emerald-600/20 focus:border-emerald-600"
                  >
                    <option value="" className="bg-white text-slate-700">-- Selecione uma sugestão de NCM --</option>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input
                        ...
                        className="w-full px-3.5 py-2.5 bg-white border border-slate-300 rounded text-xs font-mono font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-600/20 focus:border-emerald-600"
                    />
                  </div>
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <input
                        ...
                        placeholder="Ex: Tinta acrílica fosca para exteriores"
                        className="w-full px-3.5 py-2.5 bg-white border border-slate-300 rounded text-xs text-slate-900 font-medium focus:outline-none focus:ring-2 focus:ring-emerald-600/20 focus:border-emerald-600"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                  <div>
                    <label className="block text-xs uppercase tracking-wider font-semibold text-slate-600 mb-1.5 flex items-center gap-1">
                      <DollarSign className="w-3.5 h-3.5 text-emerald-700" /> Valor Produtos (R$)
                    </label>
                    <input
                        type="number"
                        min="0"
          ...
          <div className="pt-2">
            <button
                ...
                onClick={onSubmit}
                disabled={loading}
                className="w-full py-3.5 px-5 font-bold text-xs rounded-lg uppercase tracking-wider flex items-center justify-center gap-2 bg-emerald-700 hover:bg-emerald-800 text-white shadow-md transition cursor-pointer disabled:opacity-50"
            >
              {loading ? (
                  <>
